...
export const SOURCE_READINESS_MATRIX: readonly SourceReadinessDescriptor[] = [
  ...
  {
    source: "upcdev",
    requiredEnvVars: ["UPCDEV_API_KEY"],
    optionalEnvVars: [],
    categoryCoverage: "any",
    capabilities: ["catalogIdentity", "barcodeLookup"],
    liveTested: true,
    ...
    costClass: "free",
    productionAllowed: true,
    notes: "Audit des conditions d'utilisation effectué ce lot (LOT 'Live Identity Enrichment...', section 3, docs/free-open-sources-audit.md) AVANT implémentation : usage commercial explicitement permis (\"perpetual license... cache, display, and derive from the data\"), seule restriction = jamais un dump brut en masse redistribué. Palier gratuit 100 requêtes/jour avec clé en libre-service (POST /v1/auth/register). Appels réels effectués ce lot en accès public (sans clé, palier \"basic data\") : confirmé backé par Open Food Facts pour au moins un produit testé (image_url -> images.openfoodfacts.org) — chevauchement réel avec open_food_facts, d'où son rang de repli de SECOND RANG (voir identity-source-routing.ts, jamais consulté avant OFF/OPF/Wikidata). Un appel AUTHENTIFIÉ réel avec UPCDEV_API_KEY n'a pas pu être effectué (clé absente de cet environnement).",
    freeClass: "free_key_required",
  },
  {
    source: "open_prices",
