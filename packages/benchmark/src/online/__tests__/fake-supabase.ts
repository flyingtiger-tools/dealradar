/**
 * Faux client Supabase pour les tests du mode --online de packages/benchmark
 * — miroir de packages/ingestion/src/__tests__/fake-supabase.ts, étendu avec
 * delete()/gte() nécessaires à cleanup.ts. Duplication volontaire d'un
 * utilitaire de test (jamais de logique de production), même esprit que les
 * autres paquets qui ont chacun leur propre fake.
 */
...
export class FakeSupabase {
  ...
  private execute(state: QueryState): { data: unknown; error: unknown } {
    ...
    const matches = (row: Row): boolean =>
      ...
      state.gteFilters.every((f) => (row[f.column] as string) >= (f.value as string)) &&
    ...
    if (state.operation === "delete") {
      const targets = rows.filter(matches);
      this.tables[state.table] = rows.filter((r) => !matches(r));
      return { data: targets, error: null };
    }
