...
function cancelRequest() {
  return new Request(`http://localhost/api/v1/analyses/${ANALYSIS_ID}/cancel`, {
    method: "POST",
...
describe("POST /v1/analyses/:id/cancel", () => {
  it("401 quand le jeton est manquant/invalide", async () => {
    vi.mocked(authenticateBearerRequest).mockResolvedValue(null);

    const response = await POST(cancelRequest(), { params: Promise.resolve({ id: ANALYSIS_ID }) });
  ...
  it("404 (jamais 403) pour l'analyse d'un autre utilisateur", async () => {
    ...
    vi.mocked(createServiceRoleClient).mockReturnValue(
      createFakeServiceRoleClient({
        onSelect: () => ({ data: { id: ANALYSIS_ID, user_id: OWNER_ID, status: "processing" }, error: null }),
      }) as never,
    );
  ...
  it("409 ALREADY_TERMINAL quand la RPC retourne false (déjà dans un état terminal)", async () => {
    vi.mocked(authenticateBearerRequest).mockResolvedValue({ userId: OWNER_ID });
    vi.mocked(createServiceRoleClient).mockReturnValue(
      createFakeServiceRoleClient({
        onSelect: () => ({ data: { id: ANALYSIS_ID, user_id: OWNER_ID, status: "completed" }, error: null }),
  ...
  it("500 quand la RPC échoue de façon inattendue", async () => {
    vi.mocked(authenticateBearerRequest).mockResolvedValue({ userId: OWNER_ID });
    vi.mocked(createServiceRoleClient).mockReturnValue(
      createFakeServiceRoleClient({
        onSelect: () => ({ data: { id: ANALYSIS_ID, user_id: OWNER_ID, status: "pending" }, error: null }),
        rpc: () => ({ data: null, error: new Error("connexion perdue") }),
      }) as never,
    );
  ...
  it("200 { id, cancelRequested: true } quand la RPC réussit pour le propriétaire", async () => {
    vi.mocked(authenticateBearerRequest).mockResolvedValue({ userId: OWNER_ID });
    vi.mocked(createServiceRoleClient).mockReturnValue(
      createFakeServiceRoleClient({
        onSelect: () => ({ data: { id: ANALYSIS_ID, user_id: OWNER_ID, status: "pending" }, error: null }),
    ...
    const json = await response.json();
    expect(json).toEqual({ id: ANALYSIS_ID, cancelRequested: true });
  });
});
